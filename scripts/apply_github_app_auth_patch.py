...
# Catalog workflow: mint a short-lived read-only installation token from the GitHub App.
path = Path(".github/workflows/catalog.yml")
text = path.read_text()
text = replace_once(
    text,
    "      LIBRARY_GITHUB_TOKEN: ${{ secrets.LIBRARY_GITHUB_TOKEN }}\n",
    "",
    "catalog PAT env",
)
text = replace_once(
    ...
    """      - uses: actions/checkout@v5
      ...
      - name: Mint Library Catalog App token
        id: catalog-app
        uses: actions/create-github-app-token@v3
        with:
          client-id: ${{ vars.LIBRARY_CATALOG_APP_CLIENT_ID }}
          private-key: ${{ secrets.LIBRARY_CATALOG_APP_PRIVATE_KEY }}
          owner: ${{ github.repository_owner }}
          permission-contents: read

""",
    "catalog app token step",
)
text = replace_once(
    text,
    """      - name: Discover GitHub APK releases
        run: |
""",
    """      - name: Discover GitHub APK releases
        env:
          LIBRARY_GITHUB_TOKEN: ${{ steps.catalog-app.outputs.token }}
        run: |
""",
    "catalog discovery token",
)
path.write_text(text)

# Release workflow: use the same read-only App token for discovery and embed only its public client ID.
path = Path(".github/workflows/release.yml")
text = path.read_text()
text = replace_once(
    text,
    "      LIBRARY_GITHUB_TOKEN: ${{ secrets.LIBRARY_GITHUB_TOKEN }}\n",
    "      LIBRARY_GITHUB_APP_CLIENT_ID: ${{ vars.LIBRARY_CATALOG_APP_CLIENT_ID }}\n",
    "release PAT env",
)
marker = """      - name: No release needed
        if: steps.version.outputs.should_release != 'true'
        run: echo "Skipping release because LIBRARY_VERSION is not higher than the latest published version."

"""
text = replace_once(
    text,
    marker,
    marker + """      - name: Mint Library Catalog App token
        id: catalog-app
        if: steps.version.outputs.should_release == 'true'
        uses: actions/create-github-app-token@v3
        with:
...
""",
    "release app token step",
)
text = replace_once(
    text,
    """      - name: Refresh catalog before build
        if: steps.version.outputs.should_release == 'true'
        run: |
""",
    """      - name: Refresh catalog before build
        ...
        run: |
""",
    "release prebuild discovery token",
)
text = replace_once(
    text,
    """      - name: Refresh catalog with the newly published Library APK
        if: steps.version.outputs.should_release == 'true'
        run: |
""",
    """      - name: Refresh catalog with the newly published Library APK
        ...
        run: |
""",
    "release postpublish discovery token",
)
path.write_text(text)
